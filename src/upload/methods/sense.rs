...
// API end point.
const SENSE_API_URL: &str = "https://gateway-api.pastel.network/";
...
pub struct SenseStorageMethod {
  client: Arc<Client>,
}

impl SenseStorageMethod {
  /// Initialize a new SenseStorageHandler.
  pub async fn new(config_data: &ConfigData) -> Result<Self> {
      if let Some(api_key) = &config_data.sense_api_key {
          ...
          let url = format!("{}/api/v1/sense/gateway_requests", SENSE_API_URL);
          ...
          match response.status() {
              ...
              StatusCode::UNAUTHORIZED => Err(anyhow!("Invalid sense api key.")),
              code => Err(anyhow!("Could not initialize sense client: {code}")),
          }
      } else {
          Err(anyhow!("Missing 'SenseApiKey' value in config file."))
      }
  }
}

#[async_trait]
impl Prepare for SenseStorageMethod {
...
#[async_trait]
impl Uploader for SenseStorageMethod {
  ...
  async fn upload(
      ...
      while !interrupted.load(Ordering::SeqCst) && !batches.is_empty() {
          ...
          let response = self
              .client
              .post(format!(
                  "{SENSE_API_URL}/api/v1/sense?make_publicly_accessible=true"
              ))
              .multipart(form)
              .send()
              .await?;
          ...
          if status.is_success() {
              ...
              for asset_info in batch {
                  ...
                  if response.results[0].original_file_ipfs_link.is_some() {
                      ...
                      match data_type {
                          DataType::Image => {
                              item.sense_id = Some(response.results[0].result_id.clone());
                              item.image_link = uri;
                          }
